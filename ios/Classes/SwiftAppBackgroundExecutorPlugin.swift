...
public class SwiftAppBackgroundExecutorPlugin: NSObject, FlutterPlugin {
    ...
    private var needCreateTaskRequest: Bool {
        get {
            SwiftAppBackgroundExecutorPlugin.flutterPluginRegistrantCallback == nil
        }
    }
    private static var instance: SwiftAppBackgroundExecutorPlugin?
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        instance = SwiftAppBackgroundExecutorPlugin()
    ...
    private func createRefreshTask(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if createdRefreshTask != request.details.taskIdentifier {
            SwiftAppBackgroundExecutorPlugin.refreshTask = request.details.taskIdentifier
            taskDelay = request.details.taskDelay
        }
        let response = CreateRefreshTaskResponse(isSuccess: true, taskIdentifier: SwiftAppBackgroundExecutorPlugin.refreshTask)
    ...
    private func runImmediatelyBackgroundTask(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        let request = parseImmediatelyTaskRequest(from: call.arguments as! [String: Any])
            FlutterEngineRunner(
                flutterPluginRegistrantCallback: SwiftAppBackgroundExecutorPlugin.flutterPluginRegistrantCallback,
    ...
    private func createTaskIfNeeded() {
        if needCreateTaskRequest {
            if createBackgroundTask(for: SwiftAppBackgroundExecutorPlugin.refreshTask) {
                createdRefreshTask = SwiftAppBackgroundExecutorPlugin.refreshTask
            }
        } else {
            if #available(iOS 13, *) {
                scheduleAppRefreshTask(for: SwiftAppBackgroundExecutorPlugin.refreshTask)
            }
        }
    }
        ...
        if #available(iOS 13.0, *) {
            let isRegistered = BGTaskScheduler.shared.register(forTaskWithIdentifier: taskIdentifier, using: DispatchQueue.global()) { (task) in
                if let callback = self.callback {
                    ...
                    let appRefreshTaskOperation = AppRefreshTaskOperation(callback: callback, flutterPluginRegistrantCallback: SwiftAppBackgroundExecutorPlugin.flutterPluginRegistrantCallback)
